...
def generate_json_with_llm(text: str):
    ...
    response_schemas = [
        ...
        ResponseSchema(name="share_class", description="The share class of the fund (e.g., GBP B (acc))"),
        ResponseSchema(name="as_of_date", description="The report date from the factsheet"),
        ResponseSchema(name="fund_overview",
                       description="General description of the fund, including strategy and target volatility",
                       type="dict"),
        ResponseSchema(name="key_facts",
                       description="Important fund details such as NAV, inception dates, fund size, etc.", type="dict"),
        ResponseSchema(name="distribution_countries", description="List of country codes where the fund is distributed",
                       type="list"),
        ResponseSchema(name="dealing_details",
                       description="Information about dealing frequency, settlement, ISIN, and investment minimums",
                       type="dict"),
        ResponseSchema(name="fees", description="Fee breakdown including entry, exit, performance, and ongoing charges",
                       type="dict"),
        ResponseSchema(name="performance", description="Annualized returns and YTD performance", type="dict"),
        ResponseSchema(name="risk_metrics",
                       description="Risk metrics including volatility, Sharpe ratio, and max drawdown", type="dict"),
        ResponseSchema(name="portfolio_characteristics",
                       description="Portfolio metrics such as exposure, liquidity profile, and number of positions",
                       type="dict"),
    ]
    ...
    prompt = PromptTemplate(
        ...
        template="""
    You are an expert in analyzing investment fund factsheets. Your task is to extract structured fund information from the provided text.

    Follow these guidelines:
    - Extract as much detail as possible based on the fields below.
    - Ensure numeric values are correctly formatted.
    - Convert percentages to numerical values where applicable.
    - If a field is missing from the document, omit it (do not return null or placeholders).
